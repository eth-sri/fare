...
def train_flow(args, d, q, priors_1, priors_2, flow_dims, clf_dims, train_loaders, valid_loaders, test_loaders, is_spline=False, return_loss_bounds=False, lb_pred_loss=0, ub_pred_loss=1, lb_kl_loss=0, ub_kl_loss=1):
    ...
    if args.save_enc:
        torch.save(flows[0][0].state_dict(), f'trained_models/flow_0_0_{args.gamma}_enc.pt')
        torch.save(flows[1][0].state_dict(), f'trained_models/flow_1_0_{args.gamma}_enc.pt')
        torch.save(clf.state_dict(), 'trained_models/clf_{args.gamma}.pt')
