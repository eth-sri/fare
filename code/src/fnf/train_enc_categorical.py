...
def train_enc_cat(args, dims, new_ft_pos, X_train, A_train, y_train, X_valid, A_valid, y_valid, X_test, A_test, y_test):
    device = args.device
    
    # X_train = torch.tensor(X_train).float().to(args.device)
    # X_valid = torch.tensor(X_valid).float().to(args.device)
    # X_test = torch.tensor(X_test).float().to(args.device)

    A_train = torch.tensor(A_train).long().to(args.device)
    A_valid = torch.tensor(A_valid).long().to(args.device)
    A_test = torch.tensor(A_test).long().to(args.device)
    
    y_train = torch.tensor(y_train).long().to(args.device)
    y_valid = torch.tensor(y_valid).long().to(args.device)
    y_test = torch.tensor(y_test).long().to(args.device)
    
    # Split based on sensitive attribute
    train_all, train_prot, train_labels = X_train, A_train, y_train
    ...
    valid_all, valid_prot, valid_labels = X_valid, A_valid, y_valid
    ...
    test_all, test_prot, test_labels = X_test, A_test, y_test
    ...
    train_loader = torch.utils.data.DataLoader(TensorDataset(X_train, y_train, A_train), batch_size=args.batch_size, shuffle=True, drop_last=True)
    valid_loader = torch.utils.data.DataLoader(TensorDataset(X_valid, y_valid, A_valid), batch_size=args.batch_size, shuffle=False, drop_last=True)
    test_loader = torch.utils.data.DataLoader(TensorDataset(X_test, y_test, A_test), batch_size=args.batch_size, shuffle=False, drop_last=True)
    ...
    # Load priors
    # TODO cache
    prior_made1 = torch.load('src/fnf/%s/made1.pt' % args.dataset, map_location=device)
    prior_made2 = torch.load('src/fnf/%s/made2.pt' % args.dataset, map_location=device)
    ...
    if args.encode:
        with torch.no_grad():
            print(dims, np.prod(dims), sum(dims))
    ...
    for epoch in range(args.n_epochs+1):
        print(f'[training classifier] epoch {epoch}/{args.n_epochs}', flush=True)
    ...
    def train_adversary(adv_epochs, lr, weight_decay, n_hidden, hidden_dim, with_test=False):
        ...
        for epoch in range(adv_epochs):
            print('[training adversary] epoch {epoch}/{args.n_epochs}', flush=True)
            modes = ['train', 'valid']
            if with_test:
                modes += ['test']
            for mode in modes:
    ...
    z_train = train_all.cpu().numpy()
    z_test = test_all.cpu().numpy()
    z_valid = valid_all.cpu().numpy()

    c_train = A_train.cpu().numpy()
    c_test = A_test.cpu().numpy()
    c_valid = A_valid.cpu().numpy()
    ...
    c_train_final = np.vstack([c_train.reshape(-1,1), c_valid.reshape(-1,1)]).reshape(-1)
    y_train_final = np.vstack([y_train.reshape(-1,1), y_valid.reshape(-1,1)]).reshape(-1)
    ...
    print( embeddings['y_train'].shape, embeddings['y_test'].shape )
    return embeddings
