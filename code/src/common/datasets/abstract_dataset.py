...
class AbstractDataset(ABC, torch.utils.data.Dataset):
    ...
    def _normalize(self, columns):
        ...
        if use_minmax:
            self.minn, self.maxx = self.X_train.min(dim=0)[0][columns], self.X_train.max(dim=0)[0][columns]

            self.X_train[:, columns] = (self.X_train[:, columns] - self.minn) / (self.maxx - self.minn)
            if not self.X_val is None:
                self.X_val[:, columns] = (self.X_val[:, columns] - self.minn) / (self.maxx - self.minn)
            if not self.X_test is None:
                self.X_test[:, columns] = (self.X_test[:, columns] - self.minn) / (self.maxx - self.minn)
        else:
