...
def parser_setup():
    ...
    parser.add_argument("--data.name", "-d", required=False)
...
if __name__ == "__main__":
    ...
    data, meta = get_dataset(config.data)

    def prep(T):
        if len(T.shape) == 1:
            T = T.reshape(-1, 1)
        return torch.tensor(T).to(config.device)

    # convert to tensordataset here
    data['train'] = [prep(T) for T in data['train']]
    data['test'] = [prep(T) for T in data['test']]
    ...
    valid_loader = DataLoader(data["test"], shuffle=False, batch_size=config.test.batch_size,
                              num_workers=num_workers) # Code should just log valid!
    ...
    if "train" in config.mode:
        ...
        logger.info(str(print(type(trainer))))
