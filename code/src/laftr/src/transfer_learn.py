...
from src.laftr.src.codebase.utils import make_dir_if_not_exist
from src.laftr.src.codebase.datasets import TransferDataset, Dataset
from src.laftr.src.codebase.models import RegularizedFairClassifier
...
from src.laftr.src.codebase.utils import get_npz_basename
...
def main(args):
    ...
    if args['transfer']['repr_name'] == 'default':
        ...
        # load reprs from src.laftr training as input
        repr_fname = os.path.join(npzdname, 'Z.npz')
        repr_dat = np.load(repr_fname)
        reprs = repr_dat['X']
