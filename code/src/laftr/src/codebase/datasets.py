import numpy as np
import collections
from src.laftr.src.codebase.metrics import *
...
class Dataset(object):
    def __init__(self, name, attr0_name, attr1_name, npzfile, gupta_data = None, seed=0, use_attr=False, load_on_init=True, y2i=None, pred_attr=False, batch_size=None, **kwargs):
        self.name = name
        self.attr0_name = attr0_name
        self.attr1_name = attr1_name
        self.npzfile = npzfile
        self.gupta_data = gupta_data
    ...
    # x_train, x_test, attr_train, attr_test, y_train, y_test 
    # (N, 102), (N, 1) etc. 
    ...
    def load(self):
        if not self.loaded:
            if self.npzfile is not None:
                dat = np.load(self.npzfile)
                self.dat = dat # useless
                ...
                n_trainval = self.gupta_data['train'][0].shape[0] 
                ...
                self.x_train = self.gupta_data['train'][0][train_idxs]
                self.attr_train = self.gupta_data['train'][1][train_idxs].reshape(-1, 1)
                self.y_train = self.gupta_data['train'][2][train_idxs].reshape(-1, 1)

                self.x_valid = self.gupta_data['train'][0][val_idxs]
                self.attr_valid = self.gupta_data['train'][1][val_idxs].reshape(-1, 1)
                self.y_valid = self.gupta_data['train'][2][val_idxs].reshape(-1, 1)

                self.x_test = self.gupta_data['test'][0]
                self.attr_test = self.gupta_data['test'][1].reshape(-1, 1)
                self.y_test = self.gupta_data['test'][2].reshape(-1, 1)

                print('acs -> laftr data done')

                if self.use_attr:
                    self.x_train = np.concatenate([self.x_train, self.attr_train], 1)
                    self.x_test = np.concatenate([self.x_test, self.attr_test], 1)
                    self.x_valid = np.concatenate([self.x_valid, self.attr_valid], 1)
                self.loaded = True
