...
import os
from src.laftr.src.codebase.metrics import DI, DP, DI_soft
...
class Trainer(object):
    ...
    def train(self, n_epochs, patience, **kwargs):
        ...
        for epoch in range(n_epochs):
            print('starting Epoch {:d}/{:d}'.format(epoch, n_epochs), flush=True)
            ...
            #print('Class DP last epoch: {:.3f}; Disc DP Bound last epoch: {:.3f}'.format(class_dp_last_ep, disc_dp_bound_last_ep), flush=True)
            ...
            print('E{:d}: trained class {:d}, trained aud {:d}'.format(epoch, trained_class, trained_aud), flush=True)
            for k in train_L:
            ...
            if not self.regbas:
                from src.laftr.src.codebase.models import WeightedDemParWassGpGan
