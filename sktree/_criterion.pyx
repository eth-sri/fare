            ...
            for c in range(maxc):
                if use_sens == 1:
                    ...
                    if condition_on_true_label == 0:
                        count_k = self.sum_total_sens_y0[k, c]
                        prob = count_k / (self.sum_total_sens_y0[k, 0] + self.sum_total_sens_y0[k, 1])
                    elif condition_on_true_label == 1:
                        count_k = self.sum_total_sens_y1[k, c]
                        prob = count_k / (self.sum_total_sens_y1[k, 0] + self.sum_total_sens_y1[k, 1])
                    else:
        ...
        for k in range(self.n_outputs):
            ...
            for c in range(maxc): # go through classes/sens values
                if use_sens == 1:
                    # return the ratio wrt S, possibly conditioned on Y
                    if condition_on_true_label == 0:
                        count_k = self.sum_left_sens_y0[k, c]
                        prob_left = count_k / (self.sum_left_sens_y0[k, 0] + self.sum_left_sens_y0[k, 1])
                    elif condition_on_true_label == 1:
                        count_k = self.sum_left_sens_y1[k, c]
                        prob_left = count_k / (self.sum_left_sens_y1[k, 0] + self.sum_left_sens_y1[k, 1])
                    else:
                ...
                if use_sens == 1:
                    # return the ratio wrt S, possibly conditioned on Y
                    if condition_on_true_label == 0:
                        count_k = self.sum_right_sens_y0[k, c]
                        prob_right = count_k / (self.sum_right_sens_y0[k, 0] + self.sum_right_sens_y0[k, 1])
                    elif condition_on_true_label == 1:
                        count_k = self.sum_right_sens_y1[k, c]
                        prob_right = count_k / (self.sum_right_sens_y1[k, 0] + self.sum_right_sens_y1[k, 1])
                    else:
...
cdef class FairGiniEO(FairGini):
    ...
    cdef double node_impurity(self) nogil:
        ...
        g_sens_y0 = self._pvt_node_impurity(1, 0) # Sens-Gini, conditioned on y=0
        g_sens_y1 = self._pvt_node_impurity(1, 1) # Sens-Gini, conditioned on y=1
        n_y0 = (self.sum_total_sens_y0[0, 0] + self.sum_total_sens_y0[0, 1])
        n_y1 = (self.sum_total_sens_y1[0, 0] + self.sum_total_sens_y1[0, 1])
        g_sens = (n_y0 * g_sens_y0 + n_y1 * g_sens_y1) / (n_y0 + n_y1)
    ...
    cdef void children_impurity(self, double* impurity_left, double* impurity_right) nogil:
        ...
        self._pvt_children_impurity(&gL_sens_y0, &gR_sens_y0, 1, 0) # Sens-Gini, conditioned on y=0
        self._pvt_children_impurity(&gL_sens_y1, &gR_sens_y1, 1, 1) # Sens-Gini, conditioned on y=1

        n_y0 = (self.sum_left_sens_y0[0, 0] + self.sum_left_sens_y0[0, 1])
        n_y1 = (self.sum_left_sens_y1[0, 0] + self.sum_left_sens_y1[0, 1])
        gL_sens = (n_y0 * gL_sens_y0 + n_y1 * gL_sens_y1) / (n_y0 + n_y1)

        n_y0 = (self.sum_right_sens_y0[0, 0] + self.sum_right_sens_y0[0, 1])
        n_y1 = (self.sum_right_sens_y1[0, 0] + self.sum_right_sens_y1[0, 1])
        gR_sens = (n_y0 * gR_sens_y0 + n_y1 * gR_sens_y1) / (n_y0 + n_y1)
