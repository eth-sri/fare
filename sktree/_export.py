...
class _BaseTreeExporter:
    ...
    def node_to_str(self, tree, node_id, criterion):
        ...
        if tree.children_left[node_id] != _tree.TREE_LEAF:
            ...
            if tree.cat_pos[tree.feature[node_id]]:
                splitval = 'TODO NodeToStr for CATS'
            else:
                splitval = round(tree.threshold[node_id], self.precision)
    ...
    def print_tree_recurse(node, depth):
        ...
        if depth <= max_depth + 1:
            ...
            if tree_.feature[node] != _tree.TREE_UNDEFINED:
                name = str(feature_names_[node])

                node_left = tree_.children_left[node]
                node_right = tree_.children_right[node]

                if tree_.cat_pos[tree_.feature[node]]:
                    left_child_fmt = "{} [node {}] L: feature {} (max={}) cats: [{}]\n"
                    right_child_fmt = "{} [node {}] R: feature {} (max={}) cats: [{}]\n"
                    maxval = tree_.cat_maxval[tree_.feature[node]]

                    catz = {'L': [], 'R': []}
                    present, side = [], [] 
                    for idx in range(8):
                        present.append(np.binary_repr(tree_.partition[node][1][idx], width=64)[::-1])
                        side.append(np.binary_repr(tree_.partition[node][0][idx], width=64)[::-1])
                    present = ''.join(present)
                    side = ''.join(side)
                    assert len(present) == 512 
                    assert len(side) == 512 
                    for idx in range(1, maxval+1):
                        if present[idx] == '1':
                            if side[idx] == '0':
                                catz['L'].append(str(idx))
                            else:
                                catz['R'].append(str(idx))

                    s_left = left_child_fmt.format(indent, node_left, name, str(maxval), ','.join(catz['L']))
                    s_right = right_child_fmt.format(indent, node_right, name, str(maxval), ','.join(catz['R']))
                else:
                    left_child_fmt = "{} [node {}] L: feature {} <= {}\n"
                    right_child_fmt = "{} [node {}] R: feature {} > {}\n"
                    splitval = "{1:.{0}f}".format(decimals, tree_.threshold[node])
                    s_left = left_child_fmt.format(indent, node_left, name, splitval)
                    s_right = right_child_fmt.format(indent, node_right, name, splitval)

                export_text.report += s_left
                export_text.report += info_fmt_left
                print_tree_recurse(tree_.children_left[node], depth + 1)

                export_text.report += s_right
